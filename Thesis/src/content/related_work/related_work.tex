...
despite being developed in 1993, still one of the most popular methods of measuring cybersickness
...
\end{itemize}
As part of the last objective, they sought to eliminate the configural approach of the MSQ allowing for computer
administation and scoring of the results~\cite{Kennedy1993}.
...
postexposure symptoms, with the further precondition that a screening of "unhealthy" subjects is required"~\cite[p.
207]{Kennedy1993} before exposure.
...
The distinction allowed a subscale to be applied to each cluster, reflecting the impact of a simulator exposure on a
...
The SSQ symptoms are rated on a four-point scale from 0 to 3, then multiplied by either 1 or 0 (omitted) according
to the weight section of table~\ref{tab:ssq-scoring}, and finally summed up in each column~\cite{Kennedy1993}.
The total and subscale scores can then be calculated using the formulas in the "Score" section of
...
However, Kennedy et al.~\cite{Kennedy1993} state that there are too few simulator-relevant symptoms to provide
adequate reliability for smaller clusters, and recommend using the three cluster solution.
Despite the general adoption of the SSQ, there are several problems with the SSQ, especially for the assessment of
...
\end{center}
Since the weights for the scoring functions are derived from 1119 pairs of MSQs collected from 9 simulator sites
shown in table~\ref{tab:ssq-calibration}, the modal position with respect to simulator sickness across simulators in
general is no indication for symptomatology, as the zero point contains between 40\% and 75\% of the observations of
the calibration dataset~\cite{Kennedy1993}.
This also means, that the modal position on the SSQ is no indication for symptomatology, and the sensitivity is at
the upper extremes of the symptomatology range~\cite{Kennedy1993}.
Kennedy et al.~\cite{Kennedy1993} conclude that the scales should not be used to distinguish among simulators without
...
criticise the usage of the Simulator Sickness Questionnaire because of its still complex structure and development
process that only involves a sample of highly trained professionals and a small amount of simulator experiments,
which both do not comply with the modern day HMD-based VR and entertainment oriented users~\cite{Sevinc2020}.
